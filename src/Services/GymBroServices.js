...
const GymBroServices = {
  ...
  async addMatch(currentUserId, matchedUserId) {
    try {
      const matchRef = doc(
        db,
        "users",
  ...
  async addReject(currentUserId, rejectedUserId) {
    try {
      const rejectRef = doc(
        db,
        "users",
  ...
  async getExcludedUsers(currentUserId) {
    try {
      const matchesSnapshot = await getDocs(
        collection(db, "users", currentUserId, "matches")
      );
      const rejectsSnapshot = await getDocs(
        collection(db, "users", currentUserId, "rejects")
      );
      const [matches, rejects] = await Promise.all([
        getDocs(matchesSnapshot.query),
        getDocs(rejectsSnapshot.query),
      ]);
      return new Set(
        matches.docs
          .map((doc) => doc.id)
          .concat(rejects.docs.map((doc) => doc.id))
      );
    } catch (error) {
  ...
  async getMatchedUsers(currentUserId) {
    try {
      ...
      const userDocs = await Promise.all(
        matchIds.map((userId) => getDoc(doc(db, "users", userId)))
      );
      return userDocs
