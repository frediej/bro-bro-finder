...
const searchUsers = async () => {
  ...
  searchResults.value = store.allUsers.filter((user) => {
    if (user.id === currentUserId || excludeIds.value.has(user.id))
      return false;
...
const connectUser = async (matchedUserId) => {
  try {
    ...
    alert(
      "Connection interest saved! If the gymbro also likes you, you may find their full details in the matches tab!"
    );
  } catch (error) {
...
const rejectUser = async (rejectedUserId) => {
  try {
    ...
    alert(
      "They had small calves anyways, you'll never have to see them again!"
    );
  } catch (error) {
    console.error("Error rejecting user:", error);
    alert(
      "They're kind of big and mogged the system, we couldn't reject them at this time"
    );
  }
};
...
<template>
    ...
    <div class="search-filters">
      ...
      <div class="form-item">
        ...
        <select v-model="filters.daysPerWeek" id="daysPerWeek">
          <option value="">Any</option>
          <option v-for="days in daysPerWeek" :key="days" :value="days">
            {{ days }}
          </option>
        </select>
      </div>
      ...
      <div class="form-item">
        ...
        <select v-model="filters.workoutDuration" id="workoutDuration">
          <option value="">Any</option>
          <option
            v-for="duration in workoutDurations"
            :key="duration"
            :value="duration"
          >
            {{ duration }}
      ...
      <div class="form-item">
        ...
        <input
          v-model="filters.deadliftWeight"
          id="deadliftWeight"
          type="number"
          placeholder="Any"
        />
      ...
      <div class="form-item">
        ...
        <select v-model="filters.goals" id="goals" multiple>
          <option value="">Any</option>
          <option v-for="goal in goalsOptions" :key="goal" :value="goal">
            {{ goal }}
          </option>
        </select>
      </div>
  ...
  <div v-if="searchResults.length" class="search-results">
    ...
    <UserCard
      ...
      :key="user.id"
      :user="user"
      @connect="connectUser(user.id)"
      @reject="rejectUser(user.id)"
    />
    <button
      @click="resetSearch"
